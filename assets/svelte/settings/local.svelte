...
<div>
  {#each $settingsLocalStorage.items as item, index(item.id)}
    <div id={"ls-"+ item.id} animate:flip={{duration: 250}} transition:fade
      class={cn("flex select-none cursor-pointer", $settingsLocalStorage.selected == item.id ? "bg-red-100 font-bold" : "odd:bg-slate-50")}
    >
      <input type="checkbox" class="hidden" id={"ls-c-"+ item.id} on:click={() => settingsLocalStorage.select(item.id) }>
      ...
      <div class="grid grid-rows-2 place-items-center">
        <NavBar {live} side={"left"} items={[
          { ...apply_item, event: {type: "function", name: () => { apply(item) }} },
          { ...remove_item, event: {type: "function", name: () => { remove(item) }} },
        ]} />
      </div>
    </div>
  {/each}
